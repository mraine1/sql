...
SELECT
    vendor.vendor_id,
    COUNT(vendor_booth_assignments.vendor_id) AS total_rentals,
    vendor.vendor_name
FROM vendor
LEFT JOIN vendor_booth_assignments
...
SELECT
    customer.customer_last_name,
    customer.customer_first_name,
    SUM(quantity * cost_to_customer_per_qty) AS total_spent_in_april_2019
FROM customer_purchases
LEFT JOIN customer
ON customer_purchases.customer_id = customer.customer_id
WHERE strftime('%m', market_date) = '04' AND strftime('%Y', market_date) = '2019'
GROUP BY customer_purchases.customer_id
ORDER BY total_spent_in_april_2019 DESC;